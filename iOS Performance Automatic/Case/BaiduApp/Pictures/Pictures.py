...
def EnterPictures():
    os.system('python -m airtest run python /Users/jayce/Jayce-iOSPerformance/Case/NetEase/Pictures/EnterPictures.air  --device iOS:///127.0.0.1:8100 --log  /Users/jayce/Desktop/Airtest')
def EndApp():
...
def run_proc():
    os.system('python -m airtest run /Users/jayce/Jayce-iOSPerformance/Case/NetEase/Pictures/Pictures.air  --device iOS:///127.0.0.1:8100 --log  /Users/jayce/Desktop/Airtest')
def mkdir(path):
    path = path.strip()
    path = path.rstrip("\\")
...
def CV(path):
    ...
    half2 =  "{     'method_name': '_find_diff_from_back_to_front',    'threshold': 0.98,         'add_to_milestone': True,      'sift': False,    'is_image_cut': True}"
    whole = 'python /Users/jayce/Code/yuting/newsreaderAuto/core/get_time.py --path %s  --methods "%s" "%s" "%s"  "%s" "%s"' %(path,half1,half2)
    os.system('python /Users/jayce/Code/yuting/newsreaderAuto/core/get_time.py --scene tuji --path %s  --methods "%s" "%s" "%s"  "%s" "%s"' %(path,half1,half2))
    print whole
