...
def run_proc():
    os.system('python -m airtest run /Users/jayce/Jayce-iOSPerformance/Case/NetEase/Jenkins/Refresh-toutiao/Refresh-toutiao.air  --device iOS:///127.0.0.1:8100 --log  /Users/jayce/Desktop/Airtest')
def mkdir(path):
    path = path.strip()
    path = path.rstrip("\\")
...
def CV(path):
    ...
    whole = 'python /Users/jayce/Code/yuting/newsreaderAuto/core/get_time.py  --scene list_refresh --path %s  --methods "%s" "%s"' %(path,half1,half2)
    os.system('python /Users/jayce/Jayce-iOSPerformance/NewsPerformance/newsreaderAuto/core/get_time.py  --scene list_refresh --path %s  --methods "%s" "%s" ' %(path,half1,half2))
    print whole
